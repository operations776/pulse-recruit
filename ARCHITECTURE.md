...
**A sessionless route needs two things, not one.** The service-role client gets it past RLS; `CALLBACK_PATHS` in `src/middleware.ts` gets it past the session gate. The matcher is an exclusion list that catches every path, so a machine caller not named there receives the middleware's own 401 and its handler never runs. PLS-88 shipped without `/api/cron` in that list, and the deployed publisher refused its own scheduler: pg_cron would have reported a healthy job while nothing was published. Both refusals are 401, so the body is what tells them apart, "Sign in to continue" against "unauthorised", and `e2e/publish.spec.ts` asserts the second.
