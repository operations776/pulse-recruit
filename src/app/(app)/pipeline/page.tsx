...
import { getFirstJobId } from "@/lib/data";
...
//
// It used to call getWorkspace, which fetches every job, the credit ledger and
// the whole membership list, to read one id and throw the rest away. On the
// app's most-visited route that was three queries and a payload for nothing.
export default async function PipelineIndex() {
  const firstJobId = await getFirstJobId();

  if (firstJobId) redirect(`/pipeline/${firstJobId}`);
