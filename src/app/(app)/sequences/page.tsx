...
import type { Sequence, SignalKind } from "@/lib/types";
...
// on a bad monitor.
const STATUS: Record<Sequence["status"], { tone: Tone; word: string }> = {
...
export default function SequencesPage() {
  const { state, setSequenceStatus, updateStep } = useStore();
            ...
            <div className="flex flex-wrap items-start justify-between gap-4 border-b border-rule px-6 py-5">
              <div className="min-w-0">
                ...
                <p className="mt-2 max-w-[62ch] text-[12px] text-ink-2">
                  {selected.signalTrigger
                    ? TRIGGER[selected.signalTrigger]
                    : "Runs manually. Nothing sends until you enrol people yourself."}
                </p>

                <p className="meta mt-2 text-ink-3">
                  {selected.mailboxIds.length === 0
                    ? "No mailbox attached yet"
                    : `Sends from ${state.mailboxes
                        .filter((m) => selected.mailboxIds.includes(m.id))
                        .map((m) => m.address)
                        .join(", ")}`}
                </p>
              </div>
              ...
              {selected.status === "running" ? (
                <Button
                  onClick={() => {
                    setSequenceStatus(selected.id, "paused");
                    notify(
                      `${selected.name} is paused. Nothing further sends until you start it again.`,
                    );
                  }}
                >
                ...
                <Button
                  variant="primary"
                  onClick={() => {
                    setSequenceStatus(selected.id, "running");
                    notify(`${selected.name} is running.`);
                  }}
                >
