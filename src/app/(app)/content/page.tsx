...
import type { ContentPost, ContentSkill } from "@/lib/types";

type Status = ContentPost["status"];
...
const MOVE_NOTE: Record<Status, string> = {
...
export default function ContentPlannerPage() {
  const { state, addPost, setPostStatus } = useStore();
  ...
  const submit = () => {
    ...
    addPost(skill, line);
    notify(`${SKILL_BY_KEY[skill].name} added to Idea`);
    closeDialog();
  };

  const move = (post: ContentPost, status: Status) => {
          ...
          <div className="flex items-start justify-between gap-3 border-b border-rule p-5">
            <div className="min-w-0">
              ...
              <p className="meta mt-2 text-ink-3">
                ...
                {` · added ${formatDate(open.createdAt)}`}
              </p>
            </div>
            <button
